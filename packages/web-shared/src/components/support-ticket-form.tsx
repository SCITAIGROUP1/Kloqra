...
import { CheckCircle2, ChevronRight, ChevronLeft, Bug, CreditCard, BarChart2, Lightbulb, AlertTriangle, Shield, MessageSquare } from "lucide-react";
import { Button, Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@kloqra/ui";
...
function BugFields({ register }: { register: (key: string, value: string) => void }) {
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <Label>Steps to Reproduce <span className="text-red-500">*</span></Label>
        <textarea required rows={3} className={TEXTAREA_CLASS} placeholder="1. Go to..&#10;2. Click...&#10;3. See error" onChange={(e) => register("stepsToReproduce", e.target.value)} />
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-2">
          <Label>Expected Behavior</Label>
          <textarea rows={2} className={TEXTAREA_CLASS} onChange={(e) => register("expectedBehavior", e.target.value)} />
        </div>
        <div className="space-y-2">
          <Label>Actual Behavior</Label>
          <textarea rows={2} className={TEXTAREA_CLASS} onChange={(e) => register("actualBehavior", e.target.value)} />
        </div>
      </div>
      <div className="space-y-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <textarea rows={3} className={TEXTAREA_CLASS} placeholder="Describe the business problem this would solve..." onChange={(e) => register("businessImpact", e.target.value)} />
      </div>
    </div>
  );
}
...
function SecurityFields({ register }: { register: (key: string, value: string) => void }) {
  return (
    <div className="space-y-4 border border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/20 rounded-lg p-4">
      <p className="text-sm text-rose-700 dark:text-rose-400 font-medium">⚡ Security tickets are treated as Critical priority and receive a 15-minute SLA response.</p>
      <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label>Affected Area / Account <span className="text-red-500">*</span></Label>
        <Input required placeholder="e.g. My account, workspace XYZ..." onChange={(e) => register("affectedArea", e.target.value)} />
      </div>
    </div>
  );
}
  ...
  if (step === "success") {
    return (
      <div className="text-center py-8 space-y-4">
        ...
        <Button variant="link" onClick={() => { setStep("type"); setSelectedType(null); setSubject(""); setBody(""); setMeta({}); setError(""); }}>
    ...
    return (
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Choose the category that best fits your issue</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {TICKET_TYPES.map((t) => (
            ...
            >
              <span className="mt-0.5 shrink-0 bg-background/50 p-1.5 rounded-lg border border-border/50">{t.icon}</span>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${selectedType!.color}`}>
          {selectedType!.icon}
          {selectedType!.label}
        </span>
      ...
      <div className="space-y-2">
        ...
        <Input required value={subject} onChange={(e) => setSubject(e.target.value)}
          placeholder={
            selectedType?.key === "BUG_REPORT" ? "e.g. Time log not saving on mobile"
            : selectedType?.key === "BILLING" ? "e.g. Charged twice for March"
      ...
      <div className="space-y-2">
        ...
        <textarea required rows={4} className={TEXTAREA_CLASS} value={body} onChange={(e) => setBody(e.target.value)}
