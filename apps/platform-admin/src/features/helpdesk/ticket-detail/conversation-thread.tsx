          ...
          {ticket.messages && ticket.messages.length > 0 ? (
            ticket.messages.map((msg: any) => (
              <div key={msg.id} className={`flex gap-4 ${msg.senderType === "AGENT" ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`flex-1 p-4 rounded-lg border ${msg.senderType === "AGENT" ? "bg-primary/5 border-primary/20" : "bg-muted/30"}`}>
                  <div className={`flex items-center gap-2 mb-2 ${msg.senderType === "AGENT" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`text-sm text-foreground/90 whitespace-pre-wrap ${msg.senderType === "AGENT" ? "text-right" : ""}`}>
                    {msg.body}
                  </div>
                </div>
              </div>
