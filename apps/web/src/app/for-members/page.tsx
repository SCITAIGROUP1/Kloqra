  ...
  return (
    <div className="glass-card p-8 rounded-2xl border-l-4 border-l-primary relative overflow-hidden">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div
          className={`w-3 h-3 rounded-full shadow-[0_0_10px] ${running ? "bg-green-400 shadow-green-400/60 animate-pulse" : "bg-muted"}`}
        />
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-premium">
      ...
      <div className="flex gap-2 items-end h-40">
        {days.map((d, i) => (
          <div key={d} className="flex-1 flex flex-col items-center gap-2">
            ...
            />
            <span
              className={`text-xs font-medium ${i === new Date().getDay() - 1 ? "text-primary" : "text-muted-foreground"}`}
            >
              {d}
            </span>
          </div>
      ...
      <div className="mt-4 flex gap-2 text-xs">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded bg-primary/30 inline-block" />
          Logged
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded bg-warning/40 inline-block" />
          Under goal
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded bg-muted/30 inline-block" />
          Weekend
        </span>
      </div>
    </div>
  ...
  return (
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-success space-y-4">
      ...
      {projects.map((p) => (
        <div
          key={p.name}
          className="bg-background/50 rounded-xl p-4 border border-border/50 hover:border-primary/30 transition-colors cursor-pointer"
        >
  ...
  return (
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-warning">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className={`text-xs px-3 py-1 rounded-full border font-medium ${colors[status]}`}>
          {labels[status]}
        </span>
      </div>
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-purple-500">
      ...
      <div className="bg-background/50 p-4 rounded-xl border border-border mb-4">
        ...
        <div className="text-sm text-foreground">
          {sent
            ? "Forgot to log 3h of client calls on May 14th."
            : "Forgot to log 3h of client calls on..."}
        </div>
      </div>
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-emerald-500">
      ...
      <div className="space-y-3">
        {integrations.map((int) => (
          <div
            key={int.name}
            className="flex items-center justify-between bg-background/50 p-3 rounded-xl border border-border/50 hover:border-emerald-500/30 transition-colors"
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-9 h-9 rounded-lg ${int.color} flex items-center justify-center text-white font-bold text-sm`}
              >
                {int.icon}
              </div>
              <div>
  ...
  return (
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-premium">
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          <div key={label} className="bg-background/50 p-3 rounded-xl border border-border/50">
            ...
            <div
              className={`text-xs flex items-center gap-1 ${up ? "text-success" : "text-red-400"}`}
            >
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span className="text-purple-400">AI Insight:</span> You're most productive on Fridays.
  ...
  const items = [
    {
      ...
      sub: "Sarah K. approved your June timesheet",
      time: "2m ago",
      color: "text-success"
    },
    {
      icon: "💬",
      title: "Comment on Entry",
      sub: "You have a note on 'API Redesign' entry",
      time: "14m ago",
      color: "text-primary"
    },
    {
      ...
      sub: "May timesheet needs a correction",
      time: "1h ago",
      color: "text-warning"
    },
    {
      ...
      sub: "Submit timesheet by Friday 5 PM",
      time: "3h ago",
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-warning">
      ...
      <div className="space-y-3">
        {items.map((n) => (
          <div
            key={n.title}
            className="flex items-start gap-3 bg-background/50 p-3 rounded-xl border border-border/50 hover:border-primary/20 transition-colors cursor-pointer"
          >
  ...
  return (
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-blue-400">
      <div className="flex items-center gap-4 mb-6">
        <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-primary to-premium flex items-center justify-center text-white font-bold text-2xl">
          C
        </div>
        <div>
      ...
      <div className="space-y-3">
        {[
          ...
          <div
            key={label}
            className="flex justify-between items-center bg-background/50 px-4 py-2.5 rounded-lg border border-border/50"
          >
...
function AssistantMockup() {
  const messages = [
    ...
    {
      from: "ai",
      text: "You logged **23h 40m** on API Redesign last week — that's 63% of your total time. Your utilization was above goal by 3h. 🚀"
    },
    { from: "user", text: "Submit my timesheet for approval" },
    {
      from: "ai",
      text: "Done! Your June timesheet (57h 30m) has been submitted to Sarah K. for review. You'll get a notification once it's approved."
    }
  ];
  return (
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-purple-500">
      ...
      <div className="space-y-3 mb-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-xs leading-relaxed ${m.from === "user" ? "bg-primary/10 text-primary border border-primary/20 rounded-tr-sm" : "bg-background/80 border border-border rounded-tl-sm text-foreground"}`}
            >
              {m.text.replace(/\*\*(.*?)\*\*/g, "$1")}
            </div>
          </div>
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-slate-500">
      ...
      <div className="space-y-3 mb-6">
        {[
          ...
          {
            action: "Time entry saved",
            status: online ? "Synced" : "Queued (2 pending)",
            ok: online
          },
          ...
          <div
            key={action}
            className="flex items-center justify-between bg-background/50 px-4 py-3 rounded-xl border border-border/50"
          >
      ...
      <div
        className={`text-sm rounded-xl p-4 border ${online ? "bg-success/5 border-success/20 text-success" : "bg-muted/30 border-border text-muted-foreground"}`}
      >
        {online
          ? "✓ All changes synced to server. You're up to date."
          : "Working offline. All entries saved locally and will sync automatically when reconnected."}
      </div>
    </div>
  );
}
    ...
    <div className="glass-card p-6 rounded-2xl border-l-4 border-l-indigo-500">
      ...
      <div className="space-y-3 mb-6">
        {providers.map((p) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-lg ${p.bg} text-white flex items-center justify-center text-xs font-bold`}
            >
              {p.letter}
            </div>
      ...
      <section className="relative pt-40 pb-32 overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-5xl text-center relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6">
            Your time, <span className="text-gradient">tracked automatically.</span>
          </h1>
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-10">
            Stop fighting spreadsheets. Kloqra gives every team member a powerful, focused
            experience — built to get out of the way and let you do your best work.
          </p>
        ...
        <div className="container relative z-10 mx-auto px-4 text-center max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Button size="lg" variant="outline" asChild className="h-14 px-10 text-lg glass-card">
              <Link href="/pricing">
                See Pricing <ChevronRight className="ml-1 w-5 h-5" />
              </Link>
            </Button>
          </div>
        </div>
